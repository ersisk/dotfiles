#!/usr/bin/env bash
# newsboat cache.db'den baslik + okunmamis sayisi; cache kilitli olabilir diye kopya uzerinden.
...
while read -r url tag; do
  ...
  tag=${tag//\"/}; tag=${tag#ALL/}
  # tab ayraci: baslikta bosluk var, read bolmesin
  ...
  [ -z "$title" ] && { title="(cache yok)"; unread=0; }
